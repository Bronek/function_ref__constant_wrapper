...
related to the implementation (that was the easy part). It was the result of a discovery of inconsistency
...
The `foo` object is used to instantiate a `cw<foo>`, from which a `move_only_function<int(baz_t)> fn` is constructed:
...
as a constructor parameter, then the constructor will unwrap `value` (that is, `foo` object) from `cw<foo>` and invoke `foo(baz_t)` (rather than call
